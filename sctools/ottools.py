...
def display_distance_matrix(df, method='average', figsize=(5,5), distance_field='debiased_distance'):
    ...
    df_ = distance_matrix
    _g = sns.clustermap(df_, row_linkage=row_linkage, col_linkage=row_linkage, annot=True, figsize=figsize)
...
def mds_embedding_adata(df_flat, distance_field='debiased_distance'):
    """
    MDS embedding, but returning an AnnData
    """
    ...
    # stick it into an AnnData, easie for viz
    QQQ = sc.AnnData(obs=pd.DataFrame(index=distance_matrix.index.values))
    QQQ.obsp['OT_distance'] = distance_matrix.values
    QQQ.obsm['X_mds'] = mds_emb
