...
def my_ssgsea_adata(adata: AnnData, omega:float, gs):
    """
    operates on .raw!
    """
    scores = []
    for i in tqdm.tqdm(range(adata.shape[0])):
        df = pd.Series(
                adata.raw.X[i,:].A.flatten(),
                index=adata.raw.var.index
            )
        scores.append(my_ssgsea(df, omega=0, gs=gs))
    return pd.Series(scores, index=adata.obs.index)  

def my_ssgsea(x: pd.Series, omega: float, gs, plotting=False):
    """
...
def decoupler_cell_cycle_model():
    """
    turning the Cell cycle scoring into decoupler compatible input
    """
    s_genes, g2m_genes = get_cellcycle_genes()
    model_cc = pd.concat([
        pd.DataFrame({'source': 'Sphase', 'target':s_genes, 'weight': 1}),
        pd.DataFrame({'source': 'G2Mphase', 'target': g2m_genes, 'weight': 1})
    ])
    return model_cc
